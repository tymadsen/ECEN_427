...
-- Date:              Thu Nov 12 15:02:53 2015 (by Create and Import Peripheral Wizard)
...
entity user_logic is
  ...
  (
    ...
    --USER generics added here
		USER_CLOCK_RESET_VALUE				 : std_logic_vector(31 downto 0)	:= x"000f4240"; -- 1,000,000 
...
begin
  ...
  IP2Bus_Error <= '0';
	...
	-- Create counter
	-- Use slv_reg0(0) value to enable counter (LSB = '1' to run, LSB = '0' to stop)
	counter: process (Bus2IP_Clk)
	begin
		if (Bus2IP_Resetn = '0') then
			count <= USER_CLOCK_RESET_VALUE;
			interrupt <= '0';
		elsif (Bus2IP_Clk 'event and Bus2IP_Clk = '1') then
			interrupt <= '0';
			if slv_reg0(0) = '1' then -- counter can decrement
				if count = 0 then 
					if slv_reg0(1) = '1' then -- interrupts are enabled
						interrupt <= '1';
					end if;
					if slv_reg0(2) = '1' then -- reload with delay value
						count <= slv_reg1;
					else
						count <= USER_CLOCK_RESET_VALUE;
					end if;
